...
def test_render_l1_record_returns_valid_sentence(
    ...
    assert rec.sentence
    assert "gene" in rec.spans
    assert "status" in rec.spans
...
def test_variant_span_when_present_matches_rendered_variant_surface(
    ...
    while checked < 40 and attempts < 1000:
        attempts += 1
        rec = render_l1_record("EGFR", profile, biomarkers, common, rng)
        if "variant" not in rec.spans:
            continue
        start, end = rec.spans["variant"]
...
def test_method_span_when_present_matches_common_realizations(
    ...
    while checked < 30 and attempts < 1000:
        ...
        if "method" not in rec.spans:
            continue
        start, end = rec.spans["method"]
...
def test_expression_positive_renders_variant_with_value(
    ...
    for _ in range(600):
        ...
        if rec.status != "positive":
            continue
        positive_seen += 1
        assert rec.variant_id is not None, "positive expression needs variant_id"
        assert rec.negative_form_id is None
        if rec.measurement_value is not None:
            value_substituted += 1
            lo, hi = biomarkers.get("PD-L1").variants[rec.variant_id].measurement_range
            assert lo <= rec.measurement_value <= hi
...
def test_expression_negative_draws_from_negative_forms(
    ...
    for _ in range(600):
        ...
        if rec.status != "negative":
            continue
        neg_seen += 1
        if rec.negative_form_id is None:
            continue
...
def test_clone_attribution_attaches_and_tracks_span(
    ...
    for _ in range(400):
        ...
        if rec.clone_id is None:
            assert "clone" not in rec.spans
            continue
        clone_seen += 1
        assert "clone" in rec.spans
        start, end = rec.spans["clone"]
        assert rec.sentence[start:end] in clone_realizations
        other_spans = [s for name, s in rec.spans.items() if name != "clone"]
    ...
    for _ in range(100):
        ...
        for i in range(1, len(spans)):
            assert spans[i - 1][1] <= spans[i][0], (
                f"overlapping spans in {rec.sentence!r}: {rec.spans}"
            )
...
def test_l2_suppresses_variant_and_method(
    ...
    for _ in range(300):
        ...
        assert "variant" not in rec.spans
        assert "method" not in rec.spans
        assert rec.test_method is None
